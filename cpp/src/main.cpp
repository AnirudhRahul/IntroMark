...
vector<vector<TimeRange>> findSubstrings(vector<char*> pathList, bool verbose = false){
    ...
    for(int pathIndex=1; pathIndex < pathList.size(); pathIndex++){
        ...
        if(audio[0]->sample_rate!=audio[1]->sample_rate){
            cout << audio[0]->filename << " " << audio[1]->filename << " different sample rates\n";
            continue;
        }
        ...
        vector<CommonSubArr> common_substring_list = longest_common_substring(suffixArr, lcpArr, combinedLen, audio[0]->chromaLength, 10);
        delete[] suffixArr;
        delete[] lcpArr;
        cout << "Common Substrings found " << common_substring_list.size() << endl;
        
        vector<int> lengths;
        for(CommonSubArr c: common_substring_list)
            lengths.push_back(c.length);
        sort(lengths.begin(), lengths.end());
        for(int i=1;i<=10;i++)
            cout << "Len: " << lengths[lengths.size()-i] << endl;
        ...
        int item_duration = audio[1]->item_duration;
        int delay = audio[1]->delay;
        int sample_rate = audio[1]->sample_rate;
        auto toSec = [item_duration, sample_rate](int in) {
            return (double) in * item_duration / sample_rate;
        };
        
        int mergeThreshold = 1 * item_duration;
        int offsetThreshold = 1;
        ...
        std::remove_if(common_substring_list.begin(), common_substring_list.end(),
            [](const CommonSubArr & o) { return o.length <= 3; }),
...
//        }
        ...
        for(CommonSubArr common: common_substring_list){
            TimeRange curA = (struct TimeRange){
                toSec(common.startA),
                toSec(common.startA + common.length) + delay_sec
            };
            outputRanges[0].push_back(curA);

            TimeRange curB = (struct TimeRange){
                toSec(common.startB - offset),
                toSec(common.startB - offset + common.length) + delay_sec
            };
            outputRanges[1].push_back(curB);
        }

//        double secondMergeThreshold = delay_sec/2;
...
//        }
        ...
        for(int i=0;i<2;i++){
            cout << audio[i]->filename << endl;
            for(TimeRange t: outputRanges[i])
                cout << t.start << " to " << t.end << endl;
        }
...
int main(int argc, char* argv[])
{
    ...
    for(auto path : pathList){
        if(!fs::exists(path)){
            cout << "It looks like " << path << " doesn't exist" << endl;
            return 1;
        }
    }
